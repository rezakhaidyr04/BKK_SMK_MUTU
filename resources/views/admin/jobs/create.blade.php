<x-app-layout>
    <x-slot name="header">
        <x-ui.page-header title="Buat Lowongan Baru" subtitle="Tambah lowongan kerja baru untuk perusahaan.">
            <x-slot:actions>
                ...
                <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                    ...
                    <form method="POST" action="{{ route('admin.jobs.store') }}" class="p-6 space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="qualifications" rows="3" placeholder="Contoh: Lulusan SMK Jurusan RPL, bisa PHP/JavaScript..." class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition resize-y">{{ old('qualifications') }}</textarea>
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Benefit</label>
                            <textarea name="benefits" rows="3" placeholder="Contoh: BPJS, Tunjangan Transport, Makan Siang..." class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition resize-y">{{ old('benefits') }}</textarea>
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Deskripsi Pekerjaan</label>
                            <textarea name="description" rows="5" placeholder="Deskripsi lengkap tentang pekerjaan..." class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition resize-y">{{ old('description') }}</textarea>
                        </div>
