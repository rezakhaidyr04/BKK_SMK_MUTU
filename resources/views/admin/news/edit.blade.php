        ...
        <div class="relative overflow-hidden bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 shadow-2xl">
            ...
            <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-3xl font-bold text-white mb-2">Edit Berita</h1>
                        <p class="text-purple-100">Perbarui artikel berita karir.</p>
                    </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                            <div class="px-6 py-5 border-b border-gray-100 bg-gradient-to-r from-indigo-50 to-purple-50">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 class="font-bold text-gray-900">Edit Konten Berita</h3>
                                        <p class="text-sm text-gray-600">Perbarui judul dan isi berita</p>
                                    </div>
                                </div>
                            </div>
                            <div class="p-6 space-y-5">
                                ...
                                <div>
                                    ...
                                    <div id="editor"
                                         ...
                                         class="min-h-64 w-full px-4 py-3 border border-gray-200 border-t-0 rounded-b-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition prose max-w-none"
                    ...
                    <div class="space-y-6">
                            ...
                            <div class="p-6 space-y-4">
                                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-500" id="statusText">{{ $news->is_published ? 'Dipublikasikan' : 'Draft' }}</p>
                                    </div>
                                    <label class="relative inline-flex items-center cursor-pointer">
                                        <input type="checkbox" id="is_published" name="is_published" value="1"
                                               class="sr-only peer" {{ $news->is_published ? 'checked' : '' }}
                            ...
                            <div class="p-6 space-y-4">
                                <div>
                                    ...
                                    <input type="text" name="category" value="{{ old('category', $news->category) }}"
                                           ...
                                           placeholder="Contoh: Tips Karir...">
                                </div>
                                <div>
                                    <label class="block text-sm font-semibold text-gray-700 mb-2">Thumbnail</label>
                                    @if($news->thumbnail)
                                    <img id="thumbPreview" src="{{ asset('storage/' . $news->thumbnail) }}" alt="Thumbnail saat ini"
                                         class="w-full h-32 object-cover rounded-xl mb-2 border border-gray-200">
                                    @else
                                    <img id="thumbPreview" src="" alt="" class="hidden w-full h-32 object-cover rounded-xl mb-2 border border-gray-200">
                                    @endif
                                    <div class="border-2 border-dashed border-gray-200 rounded-xl p-3 text-center hover:border-indigo-400 transition">
                                        ...
                                        <p class="text-xs text-gray-400 mt-1">Upload baru untuk mengganti · maks. 2MB</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-red-100">
                            ...
                            <div class="p-6">
                                <p class="text-sm text-gray-600 mb-4">Berita akan dihapus secara permanen.</p>
                                <form method="POST" action="{{ route('admin.news.destroy', $news) }}"
                                      onsubmit="return confirm('Yakin ingin menghapus berita ini?')">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                                            class="w-full inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-red-600 text-white text-sm font-semibold rounded-xl hover:bg-red-700 transition">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
