<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div class="flex justify-between items-center mb-8">
                <div>
                    ...
                    <p class="text-gray-600">Tunjukkan pencapaian Anda</p>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                    ...
                    <div class="p-6">
                        ...
                        <form action="{{ route('certificates.destroy', $cert->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="text-red-600 text-sm hover:text-red-700">Hapus</button>
                        </form>
                    </div>
                </div>
                @endforeach
            ...
            <div class="bg-white rounded-xl shadow-lg p-12 text-center">
                ...
                <h3 class="text-xl font-bold text-gray-900 mb-2">Belum ada sertifikat</h3>
                <p class="text-gray-600">Unggah sertifikat pertama Anda untuk memulai</p>
            </div>
            @endif
        </div>
    </div>
    ...
    <div id="uploadModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-xl p-8 max-w-md w-full mx-4">
            <h3 class="text-xl font-bold mb-4">Unggah Sertifikat</h3>
            <form action="{{ route('certificates.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    <label class="block text-sm font-medium text-gray-700 mb-2">File (PDF, JPG, PNG - Maks 5MB)</label>
                    <input type="file" name="file" required accept=".pdf,.jpg,.jpeg,.png" class="w-full px-4 py-2 border rounded-lg">
                </div>
                <div class="flex gap-3">
                    <button type="submit" class="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Unggah</button>
                    <button type="button" onclick="document.getElementById('uploadModal').classList.add('hidden')" class="flex-1 px-4 py-2 border rounded-lg hover:bg-gray-50">Batal</button>
                </div>
            </form>
        </div>
    </div>
