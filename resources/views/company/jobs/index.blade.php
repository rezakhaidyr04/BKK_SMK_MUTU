...
<div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    ...
    <div class="bg-white rounded-3xl shadow p-8 text-center">
        ...
        <a href="{{ route('dashboard') }}" class="inline-flex items-center justify-center px-6 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition">Kembali ke Dashboard</a>
    </div>
    @else
    <div class="space-y-4">
    @foreach($jobs as $job)
    <x-card class="rounded-3xl">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    ...
                    <p class="text-gray-600">{{ $job->position }} &middot; {{ ucfirst($job->status) }}</p>
                </div>
                <div class="flex flex-wrap gap-2">
                    ...
                    <a href="{{ route('company.jobs.edit', $job) }}" class="inline-flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-xl text-gray-700 hover:bg-gray-50 transition">Edit</a>
                </div>
            </div>
