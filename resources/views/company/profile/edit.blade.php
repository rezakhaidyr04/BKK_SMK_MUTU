        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    ...
                    {{-- Upload Area bulat - compact --}}
                    <div class="flex items-center gap-3">
                        <div id="logo-preview-wrap" class="w-10 h-10 rounded-full border border-dashed border-slate-300 overflow-hidden bg-slate-50 flex items-center justify-center flex-shrink-0">
                            @if($company->logo)
                                <img id="logo-preview-static" src="{{ asset('storage/' . $company->logo) }}" alt="Logo" class="w-full h-full object-cover">
                            @else
                                <span id="logo-preview-static" class="text-[10px] text-slate-400">Logo</span>
                            @endif
                        </div>
                        <label for="logo-input" class="flex-1 cursor-pointer">
                            <div class="border border-slate-200 rounded-lg bg-white px-3 py-2 flex items-center justify-center gap-2 hover:border-blue-300 hover:bg-blue-50/30 transition">
                                <svg class="w-4 h-4 text-slate-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" /></svg>
                                <div class="text-left">
                                    <p id="logo-label" class="text-xs font-semibold text-slate-600 leading-none">Klik untuk pilih logo</p>
                                    <p class="text-[10px] text-slate-400 leading-none mt-0.5">Geser & zoom, WebP bulat</p>
                                </div>
                            </div>
                    ...
                    {{-- FORM --}}
                    <form method="POST" action="{{ route('company.profile.update') }}" enctype="multipart/form-data" class="space-y-5 pt-2">
                        @csrf
                        @method('PUT')

                        <div class="rounded-xl bg-slate-50 border border-slate-200 p-4 space-y-4">
                            <p class="text-xs font-bold tracking-widest uppercase text-slate-500 flex items-center gap-2">
                                <span class="w-1 h-3 bg-blue-600 rounded-full"></span> Informasi Utama
                            </p>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div class="group">
                                    ...
                                    <div class="relative">
                                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                            <svg class="w-4 h-4 text-slate-400 group-focus-within:text-blue-500 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5"/></svg>
                                        </div>
                                        <input type="text" name="name" value="{{ old('name', $company->name ?? '') }}" class="w-full pl-10 pr-3 py-2.5 bg-white border border-slate-200 rounded-xl text-sm text-slate-900 placeholder-slate-400 shadow-sm hover:border-slate-300 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 focus:bg-white outline-none transition" placeholder="PT Nama Perusahaan" />
                                    </div>
                                ...
                                <div class="group">
                                    ...
                                    <div class="relative">
                                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                            <svg class="w-4 h-4 text-slate-400 group-focus-within:text-blue-500 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 12a4 4 0 10-8 0 4 4 0 008 0zm0 0v1.5a2.5 2.5 0 005 0V12a9 9 0 10-9 9m4.5-1.206a8.959 8.959 0 01-4.5 1.207"/></svg>
                                        </div>
                                        <input type="email" name="email" value="{{ old('email', $company->email ?? '') }}" class="w-full pl-10 pr-3 py-2.5 bg-white border border-slate-200 rounded-xl text-sm text-slate-900 placeholder-slate-400 shadow-sm hover:border-slate-300 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition" placeholder="hrd@perusahaan.com" />
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div class="rounded-xl bg-white border border-slate-200 p-4 space-y-4 shadow-sm">
                            <p class="text-xs font-bold tracking-widest uppercase text-slate-500 flex items-center gap-2">
                                <span class="w-1 h-3 bg-emerald-500 rounded-full"></span> Kontak & Lokasi
                            </p>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div class="group">
                                    <label class="flex items-center gap-1.5 text-xs font-bold tracking-wide uppercase text-slate-600 mb-1.5">
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="text" name="phone" value="{{ old('phone', $company->phone ?? '') }}" class="w-full pl-11 pr-3 py-2.5 bg-white border border-slate-200 rounded-xl text-sm text-slate-900 placeholder-slate-400 shadow-sm hover:border-slate-300 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition" placeholder="812 3456 0007" />
                                    </div>
                                </div>
                                <div class="group">
                                    <label class="flex items-center gap-1.5 text-xs font-bold tracking-wide uppercase text-slate-600 mb-1.5">
                                        <svg class="w-3.5 h-3.5 text-emerald-500" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M6 6V5a3 3 0 013-3h2a3 3 0 013 3v1h2a2 2 0 012 2v3.57A22.952 22.952 0 0110 13a22.95 22.95 0 01-8-1.43V8a2 2 0 012-2h2zm2-1a1 1 0 011-1h2a1 1 0 011 1v1H8V5zm1 5a1 1 0 011-1h.01a1 1 0 011 1v3a1 1 0 01-1 1h-.01a1 1 0 01-1-1v-3z" clip-rule="evenodd"/></svg>
                                        Industri
                                    </label>
                                    <div class="relative">
                                        <select name="industry" class="w-full pl-3 pr-9 py-2.5 bg-white border border-slate-200 rounded-xl text-sm text-slate-900 shadow-sm hover:border-slate-300 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none appearance-none transition">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div class="group">
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="url" name="website" value="{{ old('website', $company->website ?? '') }}" class="w-full pl-10 pr-3 py-2.5 bg-white border border-slate-200 rounded-xl text-sm text-slate-900 placeholder-slate-400 shadow-sm hover:border-slate-300 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition" placeholder="https://perusahaan.com" />
                                    </div>
                                </div>
                                <div class="group">
                                    <label class="flex items-center gap-1.5 text-xs font-bold tracking-wide uppercase text-slate-600 mb-1.5">
                                        <svg class="w-3.5 h-3.5 text-emerald-500" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd"/></svg>
                                        Alamat
                                    </label>
                                    <div class="relative">
                                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                            <svg class="w-4 h-4 text-slate-400 group-focus-within:text-blue-500 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/></svg>
                                        </div>
                                        <input type="text" name="address" value="{{ old('address', $company->address ?? '') }}" class="w-full pl-10 pr-3 py-2.5 bg-white border border-slate-200 rounded-xl text-sm text-slate-900 placeholder-slate-400 shadow-sm hover:border-slate-300 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition" placeholder="Cikampek, Karawang" />
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div class="rounded-xl bg-gradient-to-br from-blue-50/50 to-indigo-50/30 border border-blue-100 p-4">
                            <label class="flex items-center gap-1.5 text-xs font-bold tracking-wide uppercase text-slate-700 mb-1.5">
                            ...
                            <textarea name="description" rows="3" class="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm text-slate-900 placeholder-slate-400 shadow-sm hover:border-slate-300 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none resize-y transition" placeholder="Ceritakan visi, misi, dan budaya kerja perusahaan Anda agar pelamar lebih tertarik...">{{ old('description', $company->description ?? '') }}</textarea>
                            <p class="text-[11px] text-slate-400 mt-1.5 flex items-center gap-1"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg> Deskripsi yang menarik meningkatkan peluang dilamar 3x lipat.</p>
                        </div>

                        <div class="flex justify-end pt-1">
                            <button type="submit" class="inline-flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white text-sm font-bold rounded-xl shadow-lg shadow-blue-500/25 hover:shadow-blue-600/30 hover:-translate-y-0.5 active:translate-y-0 transition-all">
