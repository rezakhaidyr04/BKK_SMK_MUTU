...
<style>
    ...
    .badge-not_submitted { background: var(--bg-soft); color: var(--text-2); }
    .doc-preview-link {
        display: inline-flex;
        align-items: center;
        gap: 0.375rem;
...
<div style="max-width: 64rem; margin: 0 auto; padding: 2rem 1rem;">
    ...
    <div class="profile-card" style="padding: 1.75rem; margin-bottom: 1.5rem;">
        <div class="section-header flex items-center justify-between">
            <div>
                <h1 style="font-size: 1.25rem; font-weight: 700; color: var(--text); margin:0;">Profil Perusahaan</h1>
        ...
        <form method="POST" action="{{ route('company.profile.update') }}" class="space-y-4" enctype="multipart/form-data">
            ...
            <div>
                <label class="form-label">Logo Perusahaan
                    <span style="color:var(--text-3); font-weight:400; text-transform:none; letter-spacing:0;">(JPG, PNG, WebP � maks 2MB, otomatis dikompresi)</span>
                </label>
    ...
    <div id="verification" class="profile-card" style="padding: 1.75rem;">
        <div class="section-header flex items-center justify-between">
            <div>
                <h2 style="font-size: 1.125rem; font-weight: 700; color: var(--text); margin:0;">Status Akun & Verifikasi</h2>
        ...
        @if($company->mou_path)
            <div style="border-radius: 12px; background: var(--bg-soft); border: 1px solid var(--border); padding: 1rem; margin-bottom: 1.5rem;">
                <p style="font-size: 0.75rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; color: var(--text-2); margin: 0 0 0.75rem;">Dokumen Kerjasama (MoU)</p>
                <div style="display: flex; flex-wrap: wrap; gap: 0.5rem; align-items: center;">
                    ...
                    @if($company->mou_number)
                        <span style="font-size: 0.8rem; color: var(--text-2);">No: {{ $company->mou_number }}</span>
                    @endif
                </div>
            </div>
        @endif
