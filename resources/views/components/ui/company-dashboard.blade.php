        ...
        <section class="space-y-6">
            ...
            >
                ...
                <x-slot:actions>
                    ...
                    <x-ui.btn href="{{ route('company.jobs.index') }}" variant="white" size="sm" style="color: var(--navy-3); box-shadow: 0 2px 4px -1px rgba(0, 0, 0, 0.05);">Daftar Lowongan</x-ui.btn>
                </x-slot:actions>
            </x-ui.dashboard-hero>
            ...
            <div class="grid gap-6 xl:grid-cols-[1.7fr_1fr]">
                <div class="space-y-6">
                    <div class="premium-card p-6 lg:p-8 relative">
                        <div class="relative flex flex-col gap-6 xl:flex-row xl:items-start xl:justify-between" style="z-index: 10;">
                            <div style="max-width: 24rem;">
                                <div style="display: inline-flex; align-items: center; gap: 0.375rem; padding: 0.25rem 0.625rem; border-radius: 9999px; background: var(--bg-soft); border: 1px solid var(--border); margin-bottom: 0.75rem;">
                                    ...
                                    <p style="font-size: 0.65rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.1em; color: var(--text-2); margin:0;">Ringkasan</p>
                                </div>
                                <h2 style="font-size: 1.25rem; font-weight: 700; color: var(--text); margin:0;">Status: <span style="color: var(--primary);">{{ $stats['company_status'] }}</span></h2>
                        ...
                        <div style="margin-top: 1.5rem; padding: 1rem 1.25rem; border-radius: 16px; background: linear-gradient(135deg, var(--bg-soft), #f1f5f9); border: 1px solid var(--border);">
                            <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                                <div>
                                    <h3 style="font-size: 0.95rem; font-weight: 700; color: var(--text); margin:0;">Publikasikan lowongan baru</h3>
                                    <p style="margin-top: 0.125rem; font-size: 0.8rem; color: var(--text-2); margin-bottom:0;">Segera dapatkan kandidat terbaik dengan publikasi sekarang.</p>
                                </div>
                                <div class="flex flex-wrap gap-2 shrink-0">
                                ...
                                @foreach($recentJobs as $job)
                                    <div class="stat-list-item sm:flex sm:items-center sm:justify-between group">
                                        <div class="flex items-center gap-3">
                                            <div style="width: 2.25rem; height: 2.25rem; border-radius: 0.75rem; background: var(--primary-light); color: var(--primary); display: flex; align-items: center; justify-content: center; transition: all 0.2s;" class="group-hover:bg-blue-600 group-hover:text-white">
                                            ...
                                            <div>
                                                <p style="font-weight: 600; color: var(--text); font-size: 0.95rem; margin:0;" class="group-hover:text-blue-700 transition-colors">{{ $job->title }}</p>
                                    ...
                                    <div class="stat-list-item">
                                        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p style="font-weight: 600; color: var(--text); font-size: 0.95rem; margin:0;">{{ $application->user->name }}</p>
                                                    <p style="font-size: 0.7rem; font-weight: 600; color: var(--primary); background: var(--primary-light); display: inline-block; padding: 0.125rem 0.375rem; border-radius: 0.25rem; margin-top: 0.125rem; margin-bottom:0;">{{ optional($application->job)->title ?? 'Lowongan' }}</p>
                                                </div>
                                            </div>
                ...
                <div class="space-y-6">
                    <div class="premium-card p-6">
                        <div class="flex flex-col gap-5">
                            <div class="flex items-center justify-between gap-3" style="border-bottom: 1px solid var(--border); padding-bottom: 1rem;">
                                <div>
                                    <p style="font-size: 0.65rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.15em; color: var(--text-3); margin:0;">Profil</p>
                                    <h2 style="margin-top: 0.125rem; font-size: 1.125rem; font-weight: 700; color: var(--text); margin:0;">{{ $company?->name ?? Auth::user()->name }}</h2>
                                </div>
                                <div style="display: flex; align-items: center; justify-content: center; width: 2.75rem; height: 2.75rem; border-radius: 0.75rem; background: linear-gradient(135deg, #3b82f6, var(--primary-dark)); color: white; font-weight: 700; font-size: 1.125rem; box-shadow: 0 4px 6px -1px rgba(59, 130, 246, 0.3);">
                            ...
                            <div>
                                ...
                                <div class="grid gap-2 sm:grid-cols-2">
                                    <div style="border-radius: 0.75rem; border: 1px solid var(--border); background: var(--bg-soft); padding: 0.75rem;">
                                        ...
                                        <p style="margin-top: 0.125rem; font-size: 0.95rem; font-weight: 700; color: var(--text); margin:0;">{{ $stats['company_status'] }}</p>
                                    </div>
                                    <div style="border-radius: 0.75rem; border: 1px solid var(--border); background: var(--bg-soft); padding: 0.75rem;">
                                        <p style="font-size: 0.65rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.1em; color: #64748b; margin:0;">Progress</p>
                                        <p style="margin-top: 0.125rem; font-size: 0.95rem; font-weight: 700; color: var(--text); margin:0;">{{ $stats['verification_percent'] }}%</p>
                                    </div>
                                </div>
                            </div>

                            <div style="border-radius: 0.75rem; background: var(--primary-light); border: 1px solid #dbeafe; padding: 0.875rem;">
                    ...
                    <div class="premium-card p-6">
                        <div class="flex items-center justify-between gap-3 mb-4">
                            <div>
                                <h3 style="font-size: 0.95rem; font-weight: 700; color: var(--text); margin:0;">Statistik Ringkas</h3>
                            </div>
                            <x-ui.btn href="{{ route('applications.index') }}" variant="secondary" size="sm" class="px-2 py-1 text-xs">Lihat semua</x-ui.btn>
                        </div>
                        <div class="grid gap-3">
                            <div style="display: flex; align-items: center; justify-content: space-between; border-radius: 0.75rem; border: 1px solid var(--border); background: linear-gradient(135deg, var(--bg-soft), var(--white)); padding: 0.875rem;">
                                <div class="flex items-center gap-3">
                                    ...
                                    <p style="font-size: 0.8rem; font-weight: 600; color: var(--text-2); margin:0;">Lowongan Aktif</p>
                                </div>
                                <p style="font-size: 1.125rem; font-weight: 700; color: var(--text); margin:0;">{{ $stats['active_jobs'] }}</p>
                            </div>
                            <div style="display: flex; align-items: center; justify-content: space-between; border-radius: 0.75rem; border: 1px solid var(--border); background: linear-gradient(135deg, var(--bg-soft), var(--white)); padding: 0.875rem;">
                                <div class="flex items-center gap-3">
                                    ...
                                    <p style="font-size: 0.8rem; font-weight: 600; color: var(--text-2); margin:0;">Menunggu</p>
                                </div>
                                <p style="font-size: 1.125rem; font-weight: 700; color: var(--text); margin:0;">{{ $stats['pending_applications'] }}</p>
                            </div>
                        </div>
                    </div>
                </div>
