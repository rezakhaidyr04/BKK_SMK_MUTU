...
@for($i = 0; $i < $count; $i++)
    @if($type === 'stat')
    <div class="bg-white rounded-2xl border border-neutral-100 p-6 animate-pulse">
        <div class="flex items-center gap-4">
            <div class="w-14 h-14 rounded-2xl bg-neutral-200 flex-shrink-0"></div>
            <div class="flex-1 space-y-2">
                <div class="h-7 bg-neutral-200 rounded-lg w-1/2"></div>
    ...
    <div class="animate-pulse">
        <div class="bg-neutral-100 rounded-xl p-4 mb-2 flex gap-4">
        ...
        @for($r = 0; $r < ($lines ?? 5); $r++)
        <div class="flex gap-4 px-4 py-3 border-b border-neutral-100">
            <div class="h-4 bg-neutral-200 rounded w-8"></div>
            <div class="flex items-center gap-3 flex-1">
                <div class="w-8 h-8 rounded-full bg-neutral-200 flex-shrink-0"></div>
                <div class="h-4 bg-neutral-200 rounded flex-1"></div>
            </div>
            <div class="h-4 bg-neutral-200 rounded w-24"></div>
    ...
    <div class="animate-pulse space-y-6">
        <div class="flex items-center gap-5">
            <div class="w-20 h-20 rounded-full bg-neutral-200 flex-shrink-0"></div>
            <div class="flex-1 space-y-2">
                <div class="h-6 bg-neutral-200 rounded-lg w-1/3"></div>
        ...
        <div class="space-y-3">
            @for($l = 0; $l < 5; $l++)
            <div class="h-4 bg-neutral-200 rounded {{ $l % 3 === 2 ? 'w-3/4' : 'w-full' }}"></div>
            @endfor
        </div>
    </div>
    ...
    <div class="animate-pulse space-y-3">
        @for($l = 0; $l < ($lines ?? 4); $l++)
        <div class="flex items-center gap-3 p-3 rounded-xl border border-neutral-100">
            <div class="w-10 h-10 rounded-xl bg-neutral-200 flex-shrink-0"></div>
    ...
    <div class="bg-white rounded-2xl border border-neutral-100 p-6 animate-pulse">
        <div class="flex items-start gap-4">
            ...
            <div class="flex-1 min-w-0 space-y-2">
                <div class="h-4 bg-neutral-200 rounded-lg w-3/4"></div>
                <div class="h-3 bg-neutral-200 rounded w-1/2"></div>
            </div>
        </div>
        @if($lines > 1)
