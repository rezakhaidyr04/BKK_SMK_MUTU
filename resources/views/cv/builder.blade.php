<x-app-layout>
    <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-slate-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <h1 class="text-3xl font-bold text-gray-900 mb-2">CV Builder</h1>
            <p class="text-gray-600 mb-8">Create ATS-friendly CV in minutes</p>
            
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div class="lg:col-span-2 bg-white rounded-xl shadow-lg p-8">
                    <h2 class="text-xl font-bold mb-6">Choose Template</h2>
                ...
                <div class="bg-white rounded-xl shadow-lg p-6">
                    <h3 class="text-lg font-bold mb-4">Your CVs</h3>
                    @if($cvFiles->count() > 0)
                        @foreach($cvFiles as $cv)
                        <div class="mb-3 p-3 bg-gray-50 rounded-lg">
                            <p class="text-sm font-medium">CV {{ $cv->created_at->format('d M Y') }}</p>
                            <a href="{{ route('cv.download', $cv->id) }}" class="text-blue-600 text-sm">Download</a>
                        </div>
                        @endforeach
                    @else
                        <p class="text-gray-600 text-sm">No CVs yet</p>
                    @endif
                </div>
            </div>
        </div>
